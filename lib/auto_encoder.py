...
class AE(models.Model):  # AE 모델링 수행하는 역할
    ...
    def Encoder(self):
        return models.Model(inputs = self.x, outputs= self.z)
    ...
    def Decoder(self):
        ...
        y = y_layer(z)
        return models.Model(inputs = z, outputs = y)
...
def plot_loss(history, title=None):     # Loss Visualization
    ...
    plt.legend(['Training data', 'Validation data'], loc=0)
    # plt.show()
def show_ae(autoencoder):                                   # 20.06.01 auto encoder 참고자료.
...
def main():  # AE main 함수 설정
    ...
    # 학습 실행
    history = autoencoder.fit(X_train, X_train, epochs=100, batch_size=5, shuffle=True, validation_data=(X_test, X_test))
...
# <ref = https://ballentain.tistory.com/26 >